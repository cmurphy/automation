...
def create_test_project(change_id, develproject, homeproject):
    testproject = '%s:ardana-ci-%s' % (homeproject, change_id)
    repo_metadata = """
...
def cleanup(testproject=None):
    if os.path.exists('./source'):
        shutil.rmtree('./source')
    if testproject and os.path.exists('./%s' % testproject):
...
def main():
    ...
    testproject = create_test_project(change.id, develproject, homeproject)
