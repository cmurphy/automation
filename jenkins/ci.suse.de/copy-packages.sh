...
read -r home_project package <<< $( $osc request show $request_id | awk '/submit:/{split($2,x,"/"); split(x[2],y,"@"); print x[1],y[1]}' )
$osc copypac --expand $home_project $package home:comurphy:Fake:Cloud:8:A
sleep 8
